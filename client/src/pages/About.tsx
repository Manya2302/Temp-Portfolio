...
export default function About() {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20 sm:pb-8">
      <PageHeader 
        title="Narrative" 
        code="01" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-24">
        ...
        <div className="space-y-10 sm:space-y-16">
          ...
          >
            ...
            <h3 className="text-xs-mono text-[10px] sm:text-xs mb-3 sm:mb-4 text-[hsl(var(--accent-primary))]">WHO AM I?</h3>
            <div className="space-y-4 sm:space-y-6">
<p className="text-[hsl(var(--text-secondary))] leading-relaxed text-sm sm:text-base">
  ...
  I am driven by building analytical solutions that are not only technically sound but also usable and impactful—whether through dashboards that support business decisions, models that surface hidden patterns, or insights that enhance security and operational efficiency. My goal is to deliver systems that feel simple to use while performing rigorous analysis behind the scenes.
          ...
          >
            {stats.map((stat, idx) => (
              <div 
                key={stat.label}
                className="border border-[hsl(var(--grid-line))] bg-[hsl(var(--bg-secondary))] p-4 sm:p-6 relative group hover:border-[hsl(var(--accent-primary))] transition-colors"
              >
                ...
                <h4 className="text-xs-mono text-[10px] sm:text-xs text-[hsl(var(--accent-primary))] mb-2">{stat.label}</h4>
                <p className="text-[hsl(var(--text-primary))] font-display text-sm sm:text-base">{stat.value}</p>
                <div className="h-px bg-[hsl(var(--grid-line))] mt-3 sm:mt-4" />
              </div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="space-y-10 sm:space-y-16">
          {narrative.map((section, idx) => (
            ...
            >
              <span className="absolute -left-[3px] top-0 w-[5px] h-[5px] bg-[hsl(var(--accent-primary))]" />
              <h3 className="text-xs-mono text-[10px] sm:text-xs mb-3 sm:mb-4 text-[hsl(var(--accent-primary))]">{section.label}</h3>
              <p className="font-display text-base sm:text-xl md:text-2xl leading-relaxed uppercase tracking-wide">
                {section.text}
              </p>
            </motion.div>
          ))}
